...
class WindroseMonitor:
    ...
    def set_cpu_profile(self, profile: str) -> bool:
        ...
        if not self.config["cpu_profile"]["enabled"]:
            return True
        try:
            cpu_path_template = self.config["cpu_profile"]["cpu_freq_path"].replace(
                "cpu0", "cpu{}"
            )
    ...
    def check_and_update(self):
        ...
        # CPU hysteresis
        if current_count > 0:
            self.state["performance_counter"] = (
                self.state.get("performance_counter", 0) + 1
            )
            self.state["balanced_counter"] = 0
        if (
            self.state["performance_counter"] >= 2
            and self.state.get("cpu_profile") != "performance"
        ):
        ...
        if (
            self.state["balanced_counter"] >= 3
            and self.state.get("cpu_profile") != "balanced"
        ):
        ...
        try:
            while True:
                try:
                    ...
                    status_msg = f"STATUS=Active | Players: {self.state.get('player_count', 0)} | CPU: {self.state.get('cpu_profile', 'unknown')}"
